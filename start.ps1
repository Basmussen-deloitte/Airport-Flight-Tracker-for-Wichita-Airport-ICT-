...
# Or in an admin PowerShell: .\start.ps1
...
if (Test-Path "static\logo.png") {
    Write-Host "✓ Logo already downloaded" -ForegroundColor Green
} else {
    Write-Host "  Downloading logo..." -ForegroundColor Gray
    & $pythonCmd -3 download_logo.py 2>&1 | Out-Null
    if (Test-Path "static\logo.png") {
        Write-Host "✓ Logo downloaded" -ForegroundColor Green
    } else {
        Write-Host "[!] Could not download logo (continuing anyway)" -ForegroundColor Yellow
    }
}
